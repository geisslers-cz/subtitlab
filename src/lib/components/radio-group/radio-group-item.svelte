...
<RadioGroupPrimitive.Item
  ...
  class={cn(
    ...
    'cursor-pointer focus-visible:ring-[3px] disabled:cursor-not-allowed disabled:opacity-50',
    className,
  )}
  {...restProps}
>
  {#snippet children({ checked })}
    <div data-slot="radio-group-indicator" class="relative flex items-center justify-center">
      {#if checked}
        <span class="bg-primary absolute top-1/2 left-1/2 size-2 -translate-1/2 rounded-full"
        ></span>
      {/if}
    </div>
  {/snippet}
